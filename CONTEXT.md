...
| Cup phase | Cup’s in-clip sequence: next allowed trigger is `scene1` → `scene2` → `finish` → `scene1`. Only manual gating besides mapped triggers. |
...
| Auto loop | Advances via `nextStoryStep`. Pause freezes scheduling; Resume continues from the current step. Any bay button click stops autoplay (illegal cup clicks stop it but do not fire). Story `reset` fires `finish` on all three clips at once. |
| Manual reset | Bay **reset** button (`finish` trigger) resets **only that clip**. Does not run the story-wide three-clip reset. |
| Trigger gating | Court/card: any mapped trigger anytime. Cup: cup phase only (`scene1` → `scene2` → `finish`). |
...
- Parallel per-clip map as *story* source of truth — one `StepState` cursor (cup phase is gating only).
